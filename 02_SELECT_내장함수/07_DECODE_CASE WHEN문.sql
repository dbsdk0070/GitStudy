...
DECODE : 동등비교만 가능(동일 데이터 확인) <자바에서 SWITCH CASE문과 비슷>
DECODE(대상, 비교값, 같을때 처리문, 다를때 처리문);
DECODE(대상, 비교값, DECODE(), DECODE());
DECODE(대상, 비교값1, 처리문1  <else if문과 같음>
          , 비교값2, 처리문2
          , ...
          , 비교값n, 처리문n
          , 처리문n+1)
...
--이름이 박지성이면 '축구선수' 아니면 '운동선수'
SELECT NAME,
     --DECODE(NAME, '박지성', '축구선수', '운동선수') AS NAME_DECODE
  CASE NAME WHEN '박지성'
       THEN '축구선수'
       ELSE '운동선수'
   END AS NAME_DECODE -- , 구분자 대신에 THEN 키워드 씀
  FROM CUSTOMER
 ORDER BY NAME
;
-----------------------------
-- 이름이 김연아 -> 피겨스케이팅, 박세리 -> 골프, 박지성 -> 축구, 나머지는 운동선수
SELECT NAME,
       CASE NAME
