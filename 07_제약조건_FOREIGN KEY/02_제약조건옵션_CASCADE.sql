...
ALTER TABLE EMP01 DISABLE CONSTRAINT SYS_C007009;
ALTER TABLE EMP02 DISABLE CONSTRAINT SYS_C007011;
...
ALTER TABLE EMP01 ENABLE CONSTRAINT SYS_C007009;
ALTER TABLE EMP02 ENABLE CONSTRAINT SYS_C007011;
...
CREATE TABLE C_TEST_SUB (
    ...
    REFERENCES C_TEST_MAIN (MAIN_PK) ON DELETE CASCADE  --FOREIGN키를 설정할때 부모테이블의 키가 없어지면 자식테이블의 키도 삭제되게 만듦
                                                        --CASCADE에 의해서 자동으로 삭제되게 만듦
);
...
----- 메인 테이블 데이터 삭제
DELETE FROM C_TEST_MAIN WHERE MAIN_PK = 1111; --MAIIN 테이블에 1111 데이터를 삭제했는데 SUB 테이블의 데이터도 삭제됨
                                              --CASCADE에 의해서... 자동으로 삭제되게 설정함
SELECT * FROM C_TEST_MAIN;
SELECT * FROM C_TEST_SUB;
DELETE FROM C_TEST_MAIN WHERE MAIN_PK = 3333;
--===========================================

--테이블 삭제 : 부모-자녀 관계 설정된 테이블의 부모 테이블 삭제 불가
DROP TABLE C_TEST_MAIN;

--방법1 : 서브 테이블 모두 삭제 후 부모테이블 삭제
DROP TABLE C_TEST_SUB;
DROP TABLE C_TEST_MAIN;
ROLLBACK;
