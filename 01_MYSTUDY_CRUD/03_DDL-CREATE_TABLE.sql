...
-----------------------------------------------
SELECT * FROM TEST WHERE NUM = 100.45; --숫자 VS 숫자
SELECT * FROM TEST WHERE NUM = '100.45'; --숫자 VS 문자(오라클OK)
SELECT * FROM TEST WHERE NUM = '100.45A'; --숫자 VS 문자(ORA-01722: invalid number)
------------------------------------
--날짜타입 DATE는 내부에 년월일시분초 데이터 저장
SELECT DATE1, TO_CHAR(DATE1, 'YYYY-MM-DD HH24:MI:SS') FROM TEST;

--한글데이터 : 3 byte, ASCII코드 : 1 byte
SELECT * FROM TEST;
INSERT INTO TEST (STR1, STR2) VALUES ('ABCDEFGHIJ', 'ABCDEFGHIJ');
--한글 4글자 * 3BYTE = 12 : ORA-12899: value too large for column
...
--NULL 과의 연산 결과는 항상 NULL(연산의미 없음)
--NULL 값에 대한 조회는 IS NULL, IS NOT NULL 키워드로 처리
----
SELECT * FROM TEST WHERE NUM = NULL; --조회안됨(NULL 비교연산 의미없음)
SELECT * FROM TEST WHERE NUM IS NULL;
SELECT * FROM TEST WHERE NUM <> NULL; --조회안됨: <>, != : 같지않다(다르다)
SELECT * FROM TEST WHERE NUM IS NOT NULL;
--------------------
--NULL 과의 연산 처리 결과
SELECT * FROM DUAL; --DUAL : DUMMY 테이블 컬럼1개, 데이터 1개
SELECT 100 + 200, 111 + 222 FROM DUAL;
SELECT NUM, NUM + 100 FROM TEST; --NULL 과의 연산결과는 항상 NULL
---------------
--정렬 NULL
SELECT * FROM TEST ORDER BY STR2; --기본 오름차순정렬, ASC 키워드 생략 가능
SELECT * FROM TEST ORDER BY STR2 DESC; --DESC : 내림차순 정렬
-- 정렬시 오라클에서는 NULL 값을 가장 큰 값으로 처리(맨마지막 출력)
-- NULL값의 조회 순서 조정 : NULLS FIRST, NULLS LAST
SELECT * FROM TEST ORDER BY NUM;
SELECT * FROM TEST ORDER BY NUM DESC;
SELECT * FROM TEST ORDER BY NUM NULLS FIRST; --NULL값을 앞에표시
SELECT * FROM TEST ORDER BY NUM DESC NULLS LAST; --NULL갑을 맨 뒤로
-- NULL 값 처리
